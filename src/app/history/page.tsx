    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 pt-24 pb-12">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
          <div>
            <Link href="/dashboard" className="text-sm text-gray-500 dark:text-gray-400 hover:text-brand-600 dark:hover:text-brand-400 flex items-center gap-1 mb-2"><ArrowLeft className="h-4 w-4" /> Back to Dashboard</Link>
            <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-gray-100">Generation History</h1>
          </div>
        </div>
        ...
        {generations.length > 0 && (
          <div className="relative mb-6">
            ...
            <input type="text" placeholder="Search by location or content..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-3 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 dark:border-gray-700 rounded-xl text-sm dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-brand-500" />
          </div>
        )}
        ...
        {filteredGenerations.length === 0 ? (
          <Card variant="bordered" padding="lg" className="text-center py-16">
            ...
            <h3 className="text-lg font-medium text-gray-900 dark:text-gray-100 mb-1">{searchTerm ? 'No matching results' : 'No generations yet'}</h3>
            ...
            {filteredGenerations.map((gen) => (
              <Card key={gen.id} variant="bordered" padding="md" className="hover:shadow-md transition-shadow">
                ...
                <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-4">
                  <p className="text-sm text-gray-700 dark:text-gray-300 whitespace-pre-wrap line-clamp-4">{gen.output_text}</p>
                </div>
              </Card>
            ))}
          </div>
